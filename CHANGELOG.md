...
- Streaming pipeline refactor from issues #11 and #59 concluded (PRs #36–#58, #60–#62): `algorithms/streaming.py` shrunk from ~5,400 LoC to 1,041 LoC across seven focused modules (`streaming`, `streaming_orchestration`, `streaming_providers`, `streaming_prompts`, `streaming_render`, `streaming_validation`, `streaming_fallbacks`).
...
- LTI 1.3 id-token signature verification (JWKS fetch + iss/aud/exp/nonce enforcement) landed in PR #49. The blueprint remains feature-flagged off by default (`NIMA_LTI_ENABLED=false`). Closes #43.
